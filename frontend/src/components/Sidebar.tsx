        ...
        <nav className="flex-1 px-4 space-y-2 overflow-y-auto custom-scrollbar">
          ...
          <button
            ...
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${
              isActive("/") ? "bg-blue-600 text-white shadow-lg shadow-blue-900/40" : "text-slate-400 hover:bg-white/5 hover:text-white"
            }`}
          >
            <Activity size={20} />
          ...
          <div className="space-y-1">
            ...
            >
              <span className="text-[11px] font-bold uppercase tracking-[0.2em]">WA Gateway</span>
              <ChevronDown size={14} className={`transition-transform duration-300 ${openMenus.wa ? 'rotate-180' : ''}`} />
            </button>
            
            {openMenus.wa && (
              <div className="space-y-1 mt-1">
                <SubNavItem icon={MessageSquare} label="Inbox" active={isActive("/inbox")} onClick={() => handleNavClick("/inbox")} />
                <SubNavItem icon={Send} label="Kirim Pesan" active={isActive("/send")} onClick={() => handleNavClick("/send")} />
                <SubNavItem icon={Smartphone} label="Device Status" active={isActive("/add-device")} onClick={() => handleNavClick("/add-device")} />
                <SubNavItem icon={Users} label="Kontak WA" active={isActive("/contacts")} onClick={() => handleNavClick("/contacts")} />
              </div>
            )}
          </div>
          ...
          <div className="space-y-1 pt-2">
            ...
            >
              ...
              <ChevronDown size={14} className={`transition-transform duration-300 ${openMenus.ig ? 'rotate-180' : ''}`} />
            </button>
            
            {openMenus.ig && (
              <div className="space-y-1 mt-1">
                <SubNavItem icon={Instagram} label="Direct Message" active={false} onClick={() => {}} />
                <SubNavItem icon={LayoutGrid} label="Auto Comment" active={false} onClick={() => {}} />
              </div>
            )}
          </div>
          ...
          <div className="space-y-1 pt-2">
            ...
            >
              ...
              <ChevronDown size={14} className={`transition-transform duration-300 ${openMenus.fb ? 'rotate-180' : ''}`} />
            </button>
            
            {openMenus.fb && (
              <div className="space-y-1 mt-1">
                <SubNavItem icon={Facebook} label="Messenger" active={false} onClick={() => {}} />
              </div>
            )}
          </div>
        ...
        <div className="p-4 bg-slate-900/50 border-t border-white/5">
          <div className="flex items-center gap-3 p-2 rounded-xl">
            ...
            <div className="flex-1 overflow-hidden">
              <p className="text-sm font-semibold text-white truncate">Admin Rian</p>
...
const SubNavItem = ({ icon: Icon, label, active, onClick, badge }: any) => (
  <button
    ...
    className={`w-full flex items-center justify-between px-4 py-2.5 rounded-lg transition-all group ${
      active ? "bg-white/10 text-blue-400" : "text-slate-400 hover:bg-white/5 hover:text-slate-200"
    }`}
  >
    ...
    {badge && <span className="text-[9px] bg-blue-600 text-white px-1.5 py-0.5 rounded font-bold">{badge}</span>}
  </button>
);
